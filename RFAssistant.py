...
if PY2:
    ...
    from rfassistant.rfdocs.commands.download_command import (
        ...
        RobotFrameworkDownloadPackagesCommand, RobotFrameworkFetchManifestCommand,
        RobotFrameworkFetchPackageCommand
    )
    from rfassistant.commands.scan_local_data_command import (
        RobotFrameworkScanPythonLibsCommand, RobotFrameworkScanResourceFilesCommand,
        RobotFrameworkScanTestCaseFilesCommand, RobotFrameworkScanPythonLibsAndResourceFilesCommand
    )
    from rfassistant.load_data import (
        RFDataCollector, RobotFrameworkOpenItemDocCommand, RobotFrameworkLogItemCommand,
        RobotFrameworkRecollectDataCommand, RobotFrameworkGoToItemSourceCommand,
        RobotFrameworkScanCurrentFileCommand
    )
    from rfassistant.syntax_highlight import AutoSyntaxHighlight
    from rfassistant.rflint.rflint_git import (
        RobotFrameworkRflintGitStagedChangedFilesCommand,
    ...
    from .rfassistant.rfdocs.commands.show_command import (RobotFrameworkShowManifestCommand,
                                                           RobotFrameworkShowPackagesCommand)
    from .rfassistant.rfdocs.commands.validate_command import (
        RobotFrameworkValidatePackagesCommand,
        RobotFrameworkInsertIntoViewCommand
    )
    from .rfassistant.rfdocs.commands.download_command import (
        RobotFrameworkDownloadManifestCommand, RobotFrameworkFetchKeywordIntoViewCommand,
